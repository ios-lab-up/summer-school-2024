...
//
//  Created by emilio M on 26/07/24.
//
...
struct ContentView: View {
    @State private var showRealityComposer = false
    
    let colorDeFondo: Color = Color(hex: "312f2f")
    let colorDelBoton: Color = Color(hex: "#32FF89").opacity(0.7)
    let colorDelTexto: Color = Color(hex: "#FFFFFF")
    let nombreDeLaApp: String = "Home Bork"
    let nombreDelCreador: String = "Bienvenido "
    let escenaDeSpline = URL(string: "https://build.spline.design/tkDe1jL9tVoV3bxZGkB5/scene.splineswift")!
    
    var body: some View {
        if showRealityComposer{
            RealityComposerview()
        } else {
            VStack(spacing: 20) {
