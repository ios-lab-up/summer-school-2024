...
//
//  Created by Regina TG on 26/07/24.
//
...
struct ContentView: View {
    @State private var homeboorkRtg21rAb4x = false
    let colorDeFondo: Color = Color(hex: "#9E7D5E")
    let colorDelBoton: Color = Color(hex: "#32FF89").opacity(0.7)
    let colorDelTexto: Color = Color(hex: "#FFFFFF")
    let nombreDeLaApp: String = "HomeBork"
    let nombreDelCreador: String = "Incerte Nombre"
    let escenaDeSpline = URL(string: "https://build.spline.design/ZNLjrW70snIJsQTCHrQV/scene.splineswift")!
    
    var body: some View {
        if homeboorkRtg21rAb4x {
            RealityComposterView()
        } else {
            VStack(spacing: 20) {
