...
//
//  JOSE JULIAN TREJO GUZMAN 27/7/24
//
...
struct ContentView: View {
    ...
    @State private var shoesRealityComposerView = false
    let colorDeFondo: Color = Color(hex: "#F90505")
    let colorDelBoton: Color = Color(hex: "#32FF89").opacity(0.7)
    let colorDelTexto: Color = Color(hex: "#FFFFFF")
    let nombreDeLaApp: String = "Cars"
    
    
    let nombreDelCreador: String = "SPORTS"
    let escenaDeSpline = URL(string:
                                "https://build.spline.design/iCRAKgvA00q1rv1LBsX1/scene.splineswift")
    var body: some View {
        if shoesRealityComposerView {
            RealityComposerView()
        } else {
            VStack(spacing: 20) {
